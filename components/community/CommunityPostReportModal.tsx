'use client'
// 커뮤니티 게시글 신고 — SongReportModal 패턴 그대로. 같은 사유 8개.
import { useState, useEffect } from 'react'
import { createPortal } from 'react-dom'
import Image from 'next/image'
import { reportCommunityPost, REPORT_REASONS, type ReportReason } from '@/services/report.service'
...
export function CommunityPostReportModal({ postId, onClose, onSubmitted }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : '신고 접수에 실패했어요')
    } finally {
      setSubmitting(false)
    }
  }
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xl font-semibold text-white">게시글 신고</p>
          </div>
        ...
        <p className="text-xs text-zinc-500 mb-4">신고 사유를 선택해 주세요. 동일 게시글을 두 번 이상 신고해도 한 번만 접수됩니다.</p>
        ...
        <div className="space-y-1 mb-5">
          {REPORT_REASONS.map((r) => {
            ...
            return (
              ...
              >
                <span>{r}</span>
                {active && <Image src="/Check.svg" alt="" width={14} height={14} style={{ filter: 'invert(1)' }} />}
              </button>
            )
          })}
        </div>
