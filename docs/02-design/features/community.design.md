...
- **개설**: 처음부터 오픈. **1인 최대 3개**(`MAX_COMMUNITIES_PER_USER`) — 단 **관리자(`profiles.is_admin`)는 무제한**(테스트·운영용, mig 047에서 `manager_id UNIQUE` 제거·앱 레벨 가드로 이관). 개설자 자동 가입. 초과 시 `community_limit_reached`(409).
...
- `manager_id UNIQUE` 제거(047) 후 일반 유저 1인 최대 3개는 **앱 레벨 가드**(count 기반)만 — 동시요청 극단 케이스는 미방어(정상 사용 충분).
...
## 14. 정식 오픈 · 미로그인 접근 정책 (2026-07-07)

- **정식 오픈**: "준비중" 게이팅 제거(커밋 `cc6db51`) — 프로덕션 커뮤니티 탭 정상 노출. 폐쇄정책(§13) 반영된 상태로 오픈.
- **접근 매트릭스**:
  | 상태 | 읽기 | 상호작용(좋아요·댓글·투표) | 글쓰기 | 가입 |
  |---|---|---|---|---|
  | 미로그인 | ✅ (소프트 월) | ❌ 로그인 유도 | ❌ | — |
  | 로그인·비가입 | ✅ 무제한 | ❌ "먼저 가입" | ❌(작성창 숨김) | ✅ |
  | 멤버/매니저 | ✅ | ✅ | ✅ | — |
- **미로그인 소프트 월**(인스타식): 공유·SEO 위해 상세는 공개하되, **서로 다른 커뮤니티 상세 진입 3번째부터 로그인 월**. [[hooks/useGuestCommunityWall]]·[[components/community/CommunityGuestWall]].
  - 카운트 단위 = **커뮤니티 진입**(한 커뮤니티 내 글은 무제한, 한도 전 재방문·허브 제외). 무료 2개 → 3번째 새 커뮤니티 진입 시 월.
  - **sticky 차단**: 한도 초과 시 `blocked` 플래그 → 로그인/24h 리셋 전까지 **재방문·새 진입 모두 월**(허브 갔다 재진입하는 바운스 우회 차단).
  - **24h 롤링 리셋**(`localStorage`). 로그인 시(가입 안 해도) 해제. 크롤러(빈 저장소)엔 월 안 뜸 → SEO 무해.
  - 월 UI = 블러 오버레이 + 로그인 CTA(클릭 시 **월 닫고** 로그인 모달 노출) + "커뮤니티 메인으로 돌아가기".
- **미로그인 ⋮ 더보기 숨김**: 게시글 ⋮ 메뉴 항목이 전부 로그인/권한 필요라, 미로그인에겐 빈 메뉴가 열리던 문제 → **⋮ 버튼 자체를 미로그인 시 미노출**([[app/(main)/community/[id]/page.tsx]]).
