...
1. **결제 인프라** — Plus·Pro 가격 확정 후 토스/포트원/스트라이프 연동
2. **실시간 부분 재생** — MiniMax `stream:true` 응답 포맷 공개 시 MediaSource 도입
3. **곡 상세 SEO** — `/song/[id]` 전용 SSR 라우트 + 곡 sitemap 확장
4. **publishCoverImage 미저장 버그** — song.service patchToRow 매핑·DB 컬럼 추가
5. **회원가입 시점 약관 동의 체크박스**
6. **쇼츠·곡 검색** — Phase 5 기능 후보
7. **(운영) Apple Secret 만료 대응** — 2026-11-27 경 재발급
8. **(운영) 마이그레이션 017 수동 적용** — `songs.comment_count`
