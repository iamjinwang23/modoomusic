...
1. **(우선, 2026-06-03 예정) 곡별 OG 공유 미리보기** — `/song/[id]` 전용 라우트 + `generateMetadata`로 카톡·페북 공유 시 곡 커버 노출
2. **결제 인프라** — Plus·Pro 가격 확정 후 토스/포트원/스트라이프 연동
3. **실시간 부분 재생** — MiniMax `stream:true` 응답 포맷 공개 시 MediaSource 도입
4. **회원가입 시점 약관 동의 체크박스**
5. **쇼츠·곡 검색** — Phase 5 기능 후보
6. **동적 OG 이미지 생성** — `opengraph-image.tsx`로 커버+제목+로고 합성 1200×630
7. **(운영) Apple Secret 만료 대응** — 2026-11-27 경 재발급
8. **(운영, 선택) 마이그레이션 017 수동 적용** — `songs.comment_count` (미적용 시 카운트 0 고정)
