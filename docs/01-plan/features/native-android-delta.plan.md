...
- **패키지명**: `com.mono.app`(iOS bundle id와 정합). 확정 후 변경 금지(스토어 귀속).
...
Firebase 콘솔에서 안드로이드 앱(`com.mono.app`) 등록 → `google-services.json` 다운로드. FCM V1 서비스계정 키를 **EAS credentials**(`eas credentials`)에 등록(레포에 커밋 금지).
...
- **타입/이름 일관성:** `push_subscriptions.platform`·`/api/iap/revenuecat-webhook`·`com.mono.app`·`pricing.ts` android(+15%)를 iOS 계획·설계와 동일 명칭으로 사용.
