...
export function MiniPlayer() {
  ...
  // 커버 지배색(coverHue)으로 글라스 틴트를 물들임 — 어둡고 저채도로(가독성 유지). 없으면 중립 틴트.
  const coverTint = typeof np?.coverHue === 'number'
    ? `hsla(${np.coverHue}, 32%, 15%, 0.55)`
    : 'rgba(16,18,24,0.5)'
  ...
  return (
    <Pressable
      style={[styles.bar, { bottom: TAB_BAR_HEIGHT + insets.bottom - 6 }]}
      onPress={() => router.push('/player')}
    >
      {/* 글래스 표면 — 프로스티드 BlurView + 커버색 틴트(coverHue 기반) */}
      <GlassSurface preferBlur tint={coverTint} />
...
const styles = StyleSheet.create({
  ...
  // 풀폭·라운드 없음·탭바 위 밀착. 글라스 배경 + 상단 헤어라인으로 피드와 구분.
  bar: {
