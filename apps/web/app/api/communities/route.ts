...
export async function POST(req: NextRequest) {
  ...
  let body: { name?: unknown; topic?: unknown; description?: unknown; coverImage?: unknown; visibility?: unknown; joinRules?: unknown }
  ...
  const result = await createCommunity(user.id, {
    ...
    joinRules: typeof body.joinRules === 'string' ? body.joinRules : null,
  })
  if (!result.ok) {
