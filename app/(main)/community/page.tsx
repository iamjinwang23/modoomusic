    ...
    <div ref={scrollRef} className="h-full overflow-y-auto">
      <div className="max-w-[860px] mx-auto px-5 py-6 space-y-8">
        ...
        {hub === null ? (
          ...
          <>
            {hub.mine.length > 0 && (
              <Section title="내 커뮤니티" href={hub.mine.length > HUB_LIMIT.mine ? '/community/list?type=mine' : undefined}>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-x-3 gap-y-6">{hub.mine.slice(0, HUB_LIMIT.mine).map(c => <CommunityCard key={c.id} c={c} />)}</div>
              </Section>
            )}
            ...
            <Section title="인기 커뮤니티" href={hub.popular.length > HUB_LIMIT.popular ? '/community/list?type=popular' : undefined}>
              {hub.popular.length === 0 ? <Empty text="아직 커뮤니티가 없어요. 첫 커뮤니티를 만들어보세요!" /> : (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-x-3 gap-y-6">{hub.popular.slice(0, HUB_LIMIT.popular).map(c => <CommunityCard key={c.id} c={c} />)}</div>
              )}
            </Section>
            ...
            {hub.recent.length > 0 && (
              <Section title="새로 생긴 커뮤니티" href={hub.recent.length > HUB_LIMIT.recent ? '/community/list?type=new' : undefined}>
                <div className="divide-y divide-white/[0.06]">
                  {hub.recent.slice(0, HUB_LIMIT.recent).map(c => <CommunityListRow key={c.id} c={c} />)}
                </div>
              </Section>
            )}
          </>
